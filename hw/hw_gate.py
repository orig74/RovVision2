...
async def motor_cmnd_to_DShot(cmnds):
    ...
    for idx, cmd in enumerate(cmnds):
        ...
        if np.sign(cmd) == -1:
            zero_val = 1048
        cmd_dshot = zero_val + min(max(abs(round(cmd*1000)), 0), 999)
        packet = cmd_dshot << 1
        dshot_msgs[idx] = (packet << 4) | (packet ^ (packet >> 4) ^ (packet >> 8)) & 0xf
...
async def dshotmsg_to_serialbuffer(dshot_msg_l):
    serial_buff = [0]*17
    serial_buff[0] = 144    #start and code nibbles
    binary_message_list = [[0]*16 for i in range(len(dshot_msg_l))]
    for msg_indx, msg in enumerate(dshot_msg_l):
        for bit_indx, bit in enumerate(bin(msg)[2:]):
            binary_message_list[msg_indx][bit_indx] = int(bit)
